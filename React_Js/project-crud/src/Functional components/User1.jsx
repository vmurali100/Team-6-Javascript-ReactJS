...
const User1 = () => {
    ...
    const getallusers=()=>{
        axios.get("  http://localhost:3000/users1").then((response)=>{
    ...
    const adduser = () => {
        console.log(user)
        axios.post("  http://localhost:3000/users1", user).then((response) => {
    ...
    const deleteuser=(userdetails)=>{
        axios.delete("   http://localhost:3000/users1/"+ userdetails.id).then((response)=>{
            getallusers()
        })
    }
    ...
    return (
        <div>
            <form>
                ...
                <input type="text" name='dob' placeholder='dd-mm-yyyy' value={user.dob} onChange={(event) => { handlechange(event) }} /><br />
