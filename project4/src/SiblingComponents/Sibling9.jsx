...
export default class Sibling9 extends Component{
    constructor(props){
        super(props)
        this.state={
            messag:"my name is message",
            email:{
    ...
    render(){
        const {sendmessag,sendemail,sendpassword}=this.props;
        const {messag,email,password}=this.state;
        return(
            <div>
                <button onClick={()=>{sendmessag(messag)}}>send messag</button>
                <button onClick={()=>{sendemail(email)}}>send email</button>
                <button onClick={()=>{sendpassword(password)}}>send password</button>
            </div>
        )
    }
}
