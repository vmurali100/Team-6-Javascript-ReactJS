...
export default class AlbumComp1 extends Component{
    constructor(props){
        super(props)
        this.state={
            message: "Hello Albums",
            person: {
                fname:"Srinivas",
                lname:"Vidya"
            },
    ...
    render(){
        const{sendMessage,sendPerson,sendAlbums}=this.props;
        const{message,person,albums}=this.state
        return(
            <div>
                <p>Album Component1</p>
                <button onClick={()=>{sendMessage(message)}}>Send Message</button>
                <button onClick={()=>{sendPerson(person)}}>Send Person</button>
                <button onClick={()=>{sendAlbums(albums)}}>Send Albums</button>                
            </div>
        )
    }
}
