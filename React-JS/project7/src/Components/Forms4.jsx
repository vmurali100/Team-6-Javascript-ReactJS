...
const Forms4 = () => {
    ...
    const addUser = () => {
        console.log(person)
        axios.post("http://localhost:3000/Form3/", person).then((response) => {
    ...
    const deleteuser = (userDetails) => {
        axios.delete("http://localhost:3000/Form3/" + userDetails.id).then((response) => {
            getAllUsers()
    ...
    const updateuser = () => {
        axios.put("http://localhost:3000/Form3/" + person.id, person).then(() => {
            getAllUsers();
            clearUser();
            setisEdit(false)
        })
    ...
    return (
        <div class="userInfo1">
            ...
            <form className="formsInfo1">
                ...
                <input type="date" name="dateofbirth" className='2' value={person.dateofbirth} onChange={(event) => { HandleChange(event) }} /> <br />
